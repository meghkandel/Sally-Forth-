  ...
  istrm(input_stream)  // use member initializer to bind reference
{
  ...
  symtab["CR"]   =  SymTabEntry(KEYWORD,0,&doCR) ;
    ...
    symtab["<"]   =  SymTabEntry(KEYWORD,0,&doLESSERTHAN) ;
    symtab["<="]   =  SymTabEntry(KEYWORD,0,&doLESSOREQ) ;
    symtab["=="]   =  SymTabEntry(KEYWORD,0,&doEQUAL) ;
    symtab["!="]   =  SymTabEntry(KEYWORD,0,&doNOTEQUAL) ;
    symtab[">="]   =  SymTabEntry(KEYWORD,0,&doGREATOREQ) ;
...
void Sally :: doDROP (Sally *Sptr){
  Token p;
    if ( Sptr->params.size() < 1 ) {
      throw out_of_range("Drop canot be done from an empty stack") ;
    }
  p = Sptr ->params.top();
  Sptr->params.pop();
...
void Sally :: doSWAP (Sally *Sptr){
  ...
  Token p1, p2;
 if ( Sptr->params.size() < 2 ) {
    throw out_of_range("Need two  parameters to swap them ") ;
 }
...
void Sally :: doROT (Sally *Sptr){
  ...
  if ( Sptr->params.size() < 3 ) {
    throw out_of_range("Need Three  parameters to rotate them ") ;
  }
  p1 = Sptr->params.top() ;
  Sptr->params.pop() ;  
...
void   Sally :: doLESSERTHAN (Sally *Sptr){

  Token p1, p2;

  if ( Sptr->params.size() < 2 ) {
    throw out_of_range("Need two  parameters to compare three elements ") ;
  }
  ...
  bool x;
  x = (p2.m_value <  p1.m_value);
  Sptr->params.push( Token(INTEGER, x, "") ) ;
}

void   Sally :: doLESSOREQ (Sally *Sptr){

  Token p1, p2;

  if ( Sptr->params.size() < 2 ) {
    throw out_of_range("Need two  parameters to compare elements ") ;
  }
...
void   Sally :: doGREATOREQ (Sally *Sptr){
...
void   Sally :: doSET (Sally *Sptr){
  ...
  if  ( it != Sptr->symtab.end())
       throw out_of_range( "This variable already exits");
...
void   Sally :: doAT (Sally *Sptr){
  ...
  if ( Sptr->params.size() < 1 ) {
    throw out_of_range("Need one  parameters to initialize it ") ;
  }
...
void   Sally :: doEXCL (Sally *Sptr){
  ...
  map <string, SymTabEntry> :: iterator it;
   ...
   if  ( it != Sptr -> symtab.end())
    ...
    throw out_of_range( "This variable does not  exits");
...
void Sally::doAND(Sally *Sptr) {
  ...
  if ( Sptr->params.size() < 2 ) {
    throw out_of_range("Need two parameters to AND operands.") ;
  }
...
void Sally::doOR(Sally *Sptr) {
  ...
  if ( Sptr->params.size() < 2 ) {
    throw out_of_range("Need two parameters to OR the operands.") ;
  }
...
void Sally::doNOT(Sally *Sptr) {
  ...
  if ( Sptr->params.size() < 1 ) {
    throw out_of_range("Need one parameter to NOT an operand.") ;
  }
